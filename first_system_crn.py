...
# The point after which we start gathering data.
warmup_point = 20*1440




rng_customer_call = np.random.RandomState(35581)
rng_departure_from_expert = np.random.RandomState(195684)
rng_departure_from_beginner = np.random.RandomState(338008)
rng_departure_from_technical = np.random.RandomState(373589)
rng_leaving_of_customer = np.random.RandomState(1779050)
rng_special_or_normal = np.random.RandomState(2134860)
rng_probability_of_getting_tired = np.random.RandomState(2679247)
rng_probability_of_technical = np.random.RandomState(3390867)



def Exponential(lambd, rng):
    r = rng.random()
...
def starting_state():
    ...
    if Uniform(0, 1, rng_special_or_normal) > 0.6:
...
def fel_maker(future_event_list, event_type, state, data, clock, customer_special=None,
    ...
    if event_type == 'Customer Call':
        ...
        if shift == 1:
            event_time = clock + Exponential(1 / 1.1, rng_customer_call)
        elif shift == 2:
            event_time = clock + Exponential(1 / 1.1, rng_customer_call)
        else:
            event_time = clock + Exponential(1 / 1.1, rng_customer_call)
    elif event_type == 'Departure From Expert':
        event_time = clock + Exponential(1 / 3, rng_departure_from_expert)
    elif event_type == 'Departure From Beginner':
        event_time = clock + Exponential(1 / 7, rng_departure_from_beginner)
    elif event_type == 'Departure From Technical':
        event_time = clock + Exponential(0.1, rng_departure_from_technical)
    elif event_type == 'Leaving Of Tired':
        max_queue_length = state['Special Customer Queue Length'] - 1
        event_time = clock + Uniform(max_queue_length, 25, rng_leaving_of_customer)
...
def Customer_Call(future_event_list, state, clock, data, customer_special=None, customer_normal=None,
    ...
    if last_customer == 'S':
        ...
        if state['Specialist Server Status'] < 2:
            ...
            # will he get tired?
            if Uniform(0, 1, rng_probability_of_getting_tired) >= 0.85:
                # he will get tired
...
def Departure_From_Expert(future_event_list, state, clock, data, customer_special=None,
    ...
    if last_customer == 'S':
        ...
        data['Special Customers Waiting Time In System'] += clock - data['Special Customers'][customer_special][
            'Arrival Time']
        if Uniform(0, 1, rng_probability_of_technical) > 0.85:
