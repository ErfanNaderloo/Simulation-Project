...
def print_header():
    print('Event Type'.ljust(15) + '\t' + 'Event Time'.ljust(15) + '\t' +
        ...
        15) + '\t' + 'Again Call Special Customer Queue Length'.ljust(
        15) + '\t' + 'Again Call Normal Customer Queue Length'.ljust(
...
def nice_print(current_state, current_event):
    print(str(current_event['Event Type']).ljust(15) + '\t' + str(round(current_event['Event Time'], 4)).ljust(
        ...
        current_state['Again Call Special Customer Queue Length']).ljust(15) + '\t' + str(
        current_state['Again Call Normal Customer Queue Length']).ljust(15) + '\t' + str(
...
def starting_state():
    ...
    state['Again Call Special Customer Queue Length'] = 0 # To store length of special customers in again call queue
    state['Again Call Normal Customer Queue Length'] = 0 # To store length of normal customers in again call queue
    ...
    data['Last Time Again Call Special Customer Queue Length Changed'] = 0
    data['Last Time Again Call Normal Customer Queue Length Changed'] = 0
    ...
    data['Again Call Special Queue Customers'] = dict()
    data['Again Call Normal Queue Customers'] = dict()
    ...
    data['Cumulative Stats']['Again Call Special Queue Waiting Time'] = 0
    data['Cumulative Stats']['Again Call Normal Queue Waiting Time'] = 0
    ...
    data['Cumulative Stats']['Area Under Again Call Special Queue Length Curve'] = 0
    data['Cumulative Stats']['Area Under Again Call Normal Queue Length Curve'] = 0
    ...
    data['Cumulative Stats']['Max Again Call Special Customer Queue Length'] = 0
    data['Cumulative Stats']['Max Again Call Normal Customer Queue Length'] = 0
    ...
    data['Again Call Special Queue Customers All'] = set()
    data['Again Call Normal Queue Customers All'] = set()
    ...
    data['Cumulative Stats']['Max Again Call Special Customer Queue Waiting Time'] = 0
    data['Cumulative Stats']['Max Again Call Normal Customer Queue Waiting Time'] = 0
    ...
    return state, future_event_list, data, disorder

# we use this function to make our events with theirs specific features.
def fel_maker(future_event_list, event_type, state, data, clock, disorder=None, customer_special=None,
    ...
    if event_type == 'Customer Call':
        ...
        if day == disorder:
            if shift == 1:
                ...
                event_time = clock + Exponential(1 / 0.5)
            else:
...
def Customer_Call(future_event_list, state, clock, data, disorder=None, customer_special=None, customer_normal=None,
                  last_customer=None):
    # we make the next arrival here.
    if Uniform(0, 1) > 0.7:
        next_customer = 'S' + str(int(customer_special[1:]) + 1)
        fel_maker(future_event_list, 'Customer Call', state, data, clock, disorder=disorder,
    ...
    if last_customer == 'S':
        ...
        if state['Specialist Server Status'] < 2:
            ...
            fel_maker(future_event_list, 'Departure From Expert', state, data, clock, disorder=None,
            ...
            # Servers are busy, then will he use again call, wait or leave?
            if state['Special Customer Queue Length'] <= 4:
                ...
                # he won't use again call
                # will he get tired?
                if Uniform(0, 1) >= 0.85:
                    # he will get tired
                    fel_maker(future_event_list, 'Leaving Of Tired', state, data, clock, disorder=None,
                              customer_special=customer_special, customer_normal=customer_normal, last_customer='S')
            else:
                # he may use again call
                if Uniform(0, 1) >= 0.5:
                    # he will not use again call
                    data['Cumulative Stats']['Area Under Special Queue Length Curve'] += state[
                                                                                             'Special Customer Queue Length'] * (
                                                                                                 clock - data[
                                                                                             'Last Time Special Customer Queue Length Changed'])
                    ...
                    # he will use again call
                    data['Cumulative Stats']['Area Under Again Call Special Queue Length Curve'] += state['Again Call Special Customer Queue Length'] * (clock -data['Last Time Again Call Special Customer Queue Length Changed'])
                    state['Again Call Special Customer Queue Length'] += 1
                    if data['Cumulative Stats']['Max Again Call Special Customer Queue Length'] < state[
                        'Again Call Special Customer Queue Length']:
                        data['Cumulative Stats']['Max Again Call Special Customer Queue Length'] = state[
                            'Again Call Special Customer Queue Length']
                    data['Again Call Special Queue Customers'][customer_special] = clock
                    data['Again Call Special Queue Customers All'].add(customer_special)
                    data['Last Time Again Call Special Customer Queue Length Changed'] = clock
        ...
        if state['Beginner Server Status'] < 3:
            ...
            fel_maker(future_event_list, 'Departure From Beginner', state, data, clock, disorder=None,
            ...
            # will he use again call, wait or leave
            if state['Normal Customer Queue Length'] <= 4:
                ...
                if Uniform(0, 1) >= 0.5:
                    # he will not use again call
                    data['Cumulative Stats']['Area Under Normal Queue Length Curve'] += state['Normal Customer Queue Length'] * (clock - data['Last Time Normal Customer Queue Length Changed'])
                    state['Normal Customer Queue Length'] += 1
                    if data['Cumulative Stats']['Max Normal Customer Queue Length'] < state[
                        'Normal Customer Queue Length']:
                        data['Cumulative Stats']['Max Normal Customer Queue Length'] = state[
                            'Normal Customer Queue Length']
                    data['Normal Queue Customers'][customer_normal] = clock
                    data['Normal Queue Customers All'].add(customer_normal)
                    data['Last Time Normal Customer Queue Length Changed'] = clock
                    ...
                    # he will use again call
                    data['Cumulative Stats']['Area Under Again Call Normal Queue Length Curve'] += state[
                                                                                                       'Again Call Normal Customer Queue Length'] * (
                                                                                                           clock -
                                                                                                           data[
                                                                                                               'Last Time Again Call Normal Customer Queue Length Changed'])
                    state['Again Call Normal Customer Queue Length'] += 1
                    if data['Cumulative Stats']['Max Again Call Normal Customer Queue Length'] < state[
                        'Again Call Normal Customer Queue Length']:
                        data['Cumulative Stats']['Max Again Call Normal Customer Queue Length'] = state[
                            'Again Call Normal Customer Queue Length']
                    data['Again Call Normal Queue Customers'][customer_normal] = clock
                    data['Again Call Normal Queue Customers All'].add(customer_normal)
                    data['Last Time Again Call Normal Customer Queue Length Changed'] = clock


def Departure_From_Expert(future_event_list, state, clock, data, disorder=None, customer_special=None,
                          customer_normal=None,
                          last_customer=None):
    ...
    if last_customer == 'S':
        ...
        if Uniform(0, 1) > 0.85:
            ...
            if state['Technical Server Status'] < 2:
                ...
                fel_maker(future_event_list, 'Departure From Technical', state, data, clock, disorder=None,
                          customer_special=customer_special, customer_normal=customer_normal, last_customer='S')
    ...
    elif shift != 1 and state['Again Call Special Customer Queue Length'] > 0:
        # we will have again call of special customer
        # who's the first in line?
        first_customer_in_queue = min(data['Again Call Special Queue Customers'],
                                      key=data['Again Call Special Queue Customers'].get)
        # this customer starts getting service
        data['Special Customers'][first_customer_in_queue]['Time Service Begins'] = clock
        # Update queue waiting time
        data['Cumulative Stats']['Again Call Special Queue Waiting Time'] += \
            clock - data['Special Customers'][first_customer_in_queue]['Arrival Time']
        if data['Cumulative Stats']['Max Again Call Special Customer Queue Waiting Time'] < clock - \
                data['Special Customers'][first_customer_in_queue]['Arrival Time']:
            data['Cumulative Stats']['Max Again Call Special Customer Queue Waiting Time'] = clock - \
                                                                                             data['Special Customers'][
                                                                                                 first_customer_in_queue][
                                                                                                 'Arrival Time']

        # Queue length changes, so calculate the area under the current rectangle
        data['Cumulative Stats']['Area Under Again Call Special Queue Length Curve'] += \
            state['Again Call Special Customer Queue Length'] * (
                    clock - data['Last Time Again Call Special Customer Queue Length Changed'])
        state['Again Call Special Customer Queue Length'] -= 1
        # This customer no longer belongs to queue
        data['Again Call Special Queue Customers'].pop(first_customer_in_queue, None)
        # Queue length just changed. Update 'Last Time Queue Length Changed'
        data['Last Time Again Call Special Queue Length Changed'] = clock
        # Schedule its specific 'End of Service' for this customer
        fel_maker(future_event_list, 'Departure From Expert', state, data, clock, disorder=None,
                  customer_special=first_customer_in_queue,
                  customer_normal=customer_normal, last_customer='S')

    elif shift != 1 and state['Again Call Normal Customer Queue Length'] > 0:
        # we will have again call of normal customer
        # who's the first in line?
        first_customer_in_queue = min(data['Again Call Normal Queue Customers'],
                                      key=data['Again Call Normal Queue Customers'].get)
        # this customer starts getting service
        data['Normal Customers'][first_customer_in_queue]['Time Service Begins'] = clock
        # Update queue waiting time
        data['Cumulative Stats']['Again Call Normal Queue Waiting Time'] += \
            clock - data['Normal Customers'][first_customer_in_queue]['Arrival Time']
        if data['Cumulative Stats']['Max Again Call Normal Customer Queue Waiting Time'] < clock - \
                data['Normal Customers'][first_customer_in_queue]['Arrival Time']:
            data['Cumulative Stats']['Max Again Call Normal Customer Queue Waiting Time'] = clock - \
                                                                                            data['Normal Customers'][
                                                                                                first_customer_in_queue][
                                                                                                'Arrival Time']
        # Queue length changes, so calculate the area under the current rectangle
        data['Cumulative Stats']['Area Under Again Call Normal Queue Length Curve'] += \
            state['Again Call Normal Customer Queue Length'] * (
                    clock - data['Last Time Again Call Normal Customer Queue Length Changed'])
        state['Again Call Normal Customer Queue Length'] -= 1
        # This customer no longer belongs to queue
        data['Again Call Normal Queue Customers'].pop(first_customer_in_queue, None)
        # Queue length just changed. Update 'Last Time Queue Length Changed'
        data['Last Time Again Call Normal Queue Length Changed'] = clock
        # Schedule its specific 'End of Service' for this customer
        fel_maker(future_event_list, 'Departure From Expert', state, data, clock, disorder=None,
                  customer_special=customer_special,
                  customer_normal=first_customer_in_queue, last_customer='C')
...
def Departure_From_Beginner(future_event_list, state, clock, data, disorder=None, customer_special=None,
                            customer_normal=None,
                            last_customer=None):
    # what is shift number
    shift = ((clock % 1440) // 480) + 1
    ...
    if state['Normal Customer Queue Length'] > 0:
        ...
        # we will not have again calls
        state['Beginner Server Status'] -= 1


def Departure_From_Technical(future_event_list, state, clock, data, disorder=None, customer_special=None,
                             customer_normal=None,
                             last_customer=None):
    if last_customer == 'S':
...
def Leaving_Of_Tired(future_event_list, state, clock, data, disorder=None, customer_special=None, customer_normal=None,
                     last_customer=None):
    if last_customer == 'S':
...
def simulation(simulation_time):
    state, future_event_list, data, disorder = starting_state()
    month = 1
    clock = 0
    table = []
    step = 1
    ...
    while clock < simulation_time:
        if clock // 43200 == month:
            month += 1
            disorder = Month_Change()
        ...
        if clock < simulation_time:
            if current_event['Event Type'] == 'Customer Call':
                Customer_Call(future_event_list, state, clock, data, disorder=disorder,
                              customer_special=customer_special,
                              customer_normal=customer_normal, last_customer=last_customer)
            elif current_event['Event Type'] == 'Departure From Expert':
                Departure_From_Expert(future_event_list, state, clock, data, disorder=disorder,
                                      customer_special=customer_special,
                                      customer_normal=customer_normal, last_customer=last_customer)
            elif current_event['Event Type'] == 'Departure From Beginner':
                Departure_From_Beginner(future_event_list, state, clock, data, disorder=disorder,
                                        customer_special=customer_special,
                                        customer_normal=customer_normal, last_customer=last_customer)
            elif current_event['Event Type'] == 'Departure From Technical':
                Departure_From_Technical(future_event_list, state, clock, data, disorder=disorder,
                                         customer_special=customer_special,
                                         customer_normal=customer_normal, last_customer=last_customer)
            elif current_event['Event Type'] == 'Leaving Of Tired':
                Leaving_Of_Tired(future_event_list, state, clock, data, disorder=disorder,
                                 customer_special=customer_special,
                                 customer_normal=customer_normal, last_customer=last_customer)
    ...
    # this part is designed due to the problem of short simulation in which we may not have for example again call customers. This part dos not cause any difference or problem.
    if data['Again Call Special Queue Customers All'] == set():
        data['Again Call Special Queue Customers All'] = {-1}
    if data['Again Call Normal Queue Customers All'] == set():
        data['Again Call Normal Queue Customers All'] = {-1}
    ...
    special_service_starters = (len(
        ...
        data['Special Queue Customers All'].union(data['Again Call Special Queue Customers All'])))
    ...
    max_special_customer_again_call_queue_length = data['Cumulative Stats'][
        'Max Again Call Special Customer Queue Length']
    max_normal_customer_again_call_queue_length = data['Cumulative Stats'][
        'Max Again Call Normal Customer Queue Length']
    ...
    mean_special_customer_again_call_queue_length = data['Cumulative Stats'][
                                                        'Area Under Again Call Special Queue Length Curve'] / simulation_time
    mean_normal_customer_again_call_queue_length = data['Cumulative Stats'][
                                                       'Area Under Again Call Normal Queue Length Curve'] / simulation_time
    ...
    max_special_customer_again_call_queue_waiting_time = data['Cumulative Stats'][
        'Max Again Call Special Customer Queue Waiting Time']
    max_normal_customer_again_call_queue_waiting_time = data['Cumulative Stats'][
        'Max Again Call Normal Customer Queue Waiting Time']
    ...
    mean_special_customer_queue_waiting_time = data['Cumulative Stats']['Special Queue Waiting Time'] / len(
        data['Special Queue Customers All'].difference(data['Again Call Special Queue Customers All']))
    mean_normal_customer_queue_waiting_time = data['Cumulative Stats']['Normal Queue Waiting Time'] / len(
        data['Normal Queue Customers All'].difference(data['Again Call Normal Queue Customers All']))

    mean_special_customer_again_call_queue_waiting_time = data['Cumulative Stats'][
                                                              'Again Call Special Queue Waiting Time'] / len(
        data['Again Call Special Queue Customers All'])
    mean_normal_customer_again_call_queue_waiting_time = data['Cumulative Stats'][
                                                             'Again Call Normal Queue Waiting Time'] / len(
        data['Again Call Normal Queue Customers All'])
    ...
    mean_special_customer_waiting_time_in_system = data['Special Customers Waiting Time In System'] / (
        len(data['Special Queue Customers All'].difference(data['Again Call Special Queue Customers All'])))
    ...
    max_special_customer_again_call_queue_length_total.append(max_special_customer_again_call_queue_length)
    max_normal_customer_again_call_queue_length_total.append(max_normal_customer_again_call_queue_length)
    ...
    mean_special_customer_again_call_queue_length_total.append(mean_special_customer_again_call_queue_length)
    mean_normal_customer_again_call_queue_length_total.append(mean_normal_customer_again_call_queue_length)
    ...
    max_special_customer_again_call_queue_waiting_time_total.append(max_special_customer_again_call_queue_waiting_time)
    max_normal_customer_again_call_queue_waiting_time_total.append(max_normal_customer_again_call_queue_waiting_time)
